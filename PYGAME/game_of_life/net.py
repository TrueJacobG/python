...
class Net:
    def __init__(self, width, height, scale, offset):
        ...
        self.net_array = np.ndarray(shape=(self.size))
        self.offset = offset

    def random_array(self):
        for x in range(self.rows):
            for y in range(self.columns):
                self.net_array[x][y] = randint(0, 1)
    ...
    def get_neighbours(self, x, y):
        ...
        for n in range(-1, 2):
            for m in range(-1, 2):
                ...
                total += self.net_array[edge_x][edge_y]
        total -= self.net_array[x][y]
        return total
